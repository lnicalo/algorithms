...
% Metodo para un tamaño de tablero fijo
% for n = 1:N
%     tn=t(n);
%     F(:,1)=funcionec(tn,u);
%     F(:,2)=funcionec(tn+deltat/2,u + deltat/2*F(:,1));
%     F(:,3)=funcionec(tn+deltat/2,u + deltat/2*F(:,2));
%     F(:,4)=funcionec(tn+deltat,  u + deltat*F(:,3));
%     
%     u=u+deltat/6*(F(:,1) + 2*F(:,2) + 2*F(:,3) + F(:,4));
%     U(:,n+1) = u;
% end
...
[t,Y] = ode45(@funcionec,t',u0); %Invoking built-in solver 'ode45'
